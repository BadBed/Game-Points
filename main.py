...
class Point(Game_Object):
    ...
    def event(self, e):
        if e.type == pg.MOUSEBUTTONDOWN and e.button == 1:
            global playersoul
            if playersoul.point != self.index and\
                    vector_length(e.pos,(self.x, self.y)) < self.RADIUS:
...
class Player_Soul(Game_Object):
    ...
    def draw(self, screen):
        self.color_update()
        self.draw_circle(screen)
        draw.circle(screen, COLOR_WHITE, (math.ceil(self.x),
                                          math.ceil(self.y)),
    ...
    def try_catch_point(self):
        ...
        for p in list_of_points.a:
            if vector_length((p.x, p.y), (self.x, self.y)) <= \
                    self.RADIUS and self.point != p.index:
                self.catch_point(p)
